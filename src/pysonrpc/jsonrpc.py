...
class JsonRpcClient:
    ...
    def _parse_response(self, response: str, raw: bool = True) -> Dict[str, any]:
    ...
    def jsonrpc_error(
        ...
        error_message = {
            ...
            self.JSONRPC_KEY_RESP_ERROR: {
                self.JSONRPC_KEY_RESP_ERROR_CODE: error, self.JSONRPC_KEY_RESP_ERROR_MSG: message,
            }
        }
        if data:
            error_message[self.JSONRPC_KEY_RESP_ERROR][self.JSONRPC_KEY_RESP_ERROR_DATA] = data
...
class Method(MethodContainer):
    ...
    def __init__(self, name, properties = {}, exec: bool = True, client: JsonRpcClient = None) -> None:
        self._fullname = name
...
class JsonRpcEndpoint(MethodContainer):
    ...
    def _methods_from_url(self, path: str) -> List[Method]:
